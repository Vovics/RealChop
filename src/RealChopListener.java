...
public class RealChopListener implements Listener {
	  ...
	  public RealChopListener(RealChop instance) {
              ...
              if (configBlockProcessingLimit < 5 || configBlockProcessingLimit>1000) configBlockProcessingLimit=150;
              configFallingLeaves = plugin.config.getBoolean("FallingLeaves");
              //plugin.getLogger().log(Level.INFO,"Fallingleaves: " + configFallingLeaves);
              //plugin.getLogger().log(Level.INFO,"DetectBlockBreakAll: " + configDetectBlockBreakAll);
              //plugin.getLogger().log(Level.INFO,"DetectBlockBreakLeaves: " + configDetectBlockBreakLeaves);
              //plugin.getLogger().log(Level.INFO,"DetectBlockBreakLog: " + configDetectBlockBreakLog);
              //plugin.getLogger().log(Level.INFO,"BlockProcessingLimit: " + configBlockProcessingLimit);
	  ...
	  void BlockBreak(Block breakBlock, Location playerLocation) {
		  ...
		  int angle1 = 90;
		  if (direction.getX()>0) {
		  	if (angle > Math.PI * 1 / 4) angle1 = 180;
			if (angle > Math.PI * 3 / 4) angle1 = 270;
		  } else {
			  if (angle > Math.PI * 1 / 4) angle1 = 0;
			  if (angle > Math.PI * 3 / 4) angle1 = 270;
		  }
		  switch (angle1) {
		  		case 0:
		  ...
		  //player.sendMessage("angle: "+ angle1 + " " + angle2 + "^ " + angle + " x:" + direction.getX() + " z:" + direction.getZ());
				  ...
				  while (nearIt.hasNext()) {
					  ...
					  if (nearBlock.getType() == Material.LOG) {
						  if (!tree.containsKey(nearLocation)) {
							  Boolean put=false;
							  if (treeId == 0 && nearBlock.getMetadata("TreeId").isEmpty()) put = true;
							  if (treeId == 1) put = true;
							  if (treeId !=0) {
								  if (nearBlock.getMetadata("TreeId").iterator().hasNext() 
									  && nearBlock.getMetadata("TreeId").iterator().next().asInt()==treeId) {
									  put=true;
								  }
							  }
							  if (put) {
					  ...
					  if (nearBlock.getType() != Material.LOG 
							  && !isLightBlock (nearBlock.getType())
						  ) {
						  solid.put(nearLocation, nearBlock);
						  //player.sendMessage("SolidBlock : " + nearBlock.getType());
					  }
		  ...
		  breakBlock.removeMetadata("TreeId", plugin);
 		  //player.sendMessage("This tree contains " + tree.size() + " blocks, and connected " + solid.size() + " solid blocks");
		  ...
		  for (fallingDistance=1; fallingDistance<50; fallingDistance++) {
	       	  ...
	       	  if (!isLightBlock(newBlockType)) break;
		  }
		  ...
		  while (logIt.hasNext()) {
     	     ...
     	     if (horisontalDistance < 0) horisontalDistance = 0;
     	    ...
     	    //int horisontalOffset=0;
     	     int horisontalOffset=(int)Math.floor((horisontalDistance)/1.5);
	  	     ...
	  	     //player.sendMessage("horisontalDistance: "+ horisontalDistance + " verticalDistance:" + verticalDistance + " fallingdistance " + fallingDistance);
		  ...
		  while (clearIt.hasNext()) {
			  ...
			  //player.sendMessage("clearDistance: "+ clearDistance + " y:" + clearBlockLocation.getY());
		  ...
		  while (LeavesIt.hasNext()) {
				...
				if (leavesMaterial != Material.LEAVES) continue;
				
				if (treeId == 0 && !leavesBlock.getMetadata("TreeId").isEmpty()) continue;  
				//if (treeId !=0 && treeId !=1) { 
						//if (!leavesBlock.getMetadata("TreeId").iterator().hasNext()) continue; 
						//if (leavesBlock.getMetadata("TreeId").iterator().next().asInt()!=treeId) continue;
				//}
				...
				if (tree.size()<2) horisontalSpeed=0;
				if (horisontalSpeed==0) {
					Location tempLocation=leavesLocation.clone().add(0, -1, 0);
					if (tempLocation.getBlockX()!=breakBlockLocation.getBlockX()
						|| tempLocation.getBlockY()!=breakBlockLocation.getBlockY()
						|| tempLocation.getBlockZ()!=breakBlockLocation.getBlockZ())
							{ 
				...
				//FallingBlock blockFalling = world.spawnFallingBlock(leavesLocation, Material.LEAVES, (byte)(leavesBlockData | 0x4));
				blockFalling.setVelocity(direction.clone().multiply(horisontalSpeed));
		  }
	  ...
	  public void onBlockBreak(BlockBreakEvent event) {
		  ...
		  if (!
			  (
				...
				|| breakBlockType==Material.LOG
			  	|| (configDetectBlockBreakLeaves && breakBlockType==Material.LEAVES)
			  )
		 ) return;
...
/*
	  ...
	  public void onBlockDamage(BlockDamageEvent event){
		...
		Player player = event.getPlayer();
		//player.sendMessage("BlockType : " + block.getType());
		//if (block.getMetadata("TreeId").size()>0)
		//player.sendMessage("TreeId: " + block.getMetadata("TreeId").get(0).asInt());
	    //player.sendMessage("data : " + block.getData());
        //player.sendMessage("typeId : " + event.getItemInHand().getTypeId());
		if (event.getItemInHand().getTypeId()==270) {
			  if (block.getType() == Material.LOG) {
				  block.breakNaturally(); // setTypeId(0);
			  }
		}

	  if (event.getItemInHand().getTypeId()==272) {
		  Location breakBlockLocation = block.getLocation();
		  byte data=0;
		  Block b=world.getBlockAt((int)breakBlockLocation.getX(), (int)breakBlockLocation.getY()+1, (int)breakBlockLocation.getZ());
		  b.setTypeIdAndData(3, data, true);
		  for (int y = 2 ; y <= 20; y++) {
			  Block newBlock=world.getBlockAt((int)breakBlockLocation.getX(), (int)breakBlockLocation.getY()+y, (int)breakBlockLocation.getZ());
			  newBlock.setTypeIdAndData(17, data, true);
			  //data++;
			  //if (data>4) data=0;
		  }
	  }
		BlockState state=block.getState();
		 int rawData=state.getRawData();
         ...
         player.sendMessage("Damage block : " + x + " , " + y + " , " + z);
       	 player.sendMessage("typeId : " + typeId);
       	 player.sendMessage("rawData : " + rawData);
       	List<MetadataValue> meta=block.getMetadata("vovic");
       	player.sendMessage("metasize : " + meta.size());
	 ...
	 public void onPlayerMove(PlayerMoveEvent event){
		  Player player = event.getPlayer();
			 ...
			 if (x != old_x || y != old_y || z != old_z) {
				 player.sendMessage("Your Coordinates : " + x + " , " + y + " , " + z);
				 old_x = x;
				 old_y = y;
				 old_z = z;
			 }
